 ...
 *******************************************************************************/
package com.vmware.vfabric.ide.eclipse.tcserver.internal.configurator.ui;
...
public class ConfiguratorPreferencesPage extends PreferencePage implements IWorkbenchPreferencePage {
		...
		if (status[0] != null) {
			if (!status[0].isOK()) {
				...
				MessageDialog.openInformation(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(),
						"Auto Configuration", status[0].getMessage());
			}
	...
	private void initWidgets() {
		...
		if (searchLocationsLabel != null) {
			searchLocationsLabel.setText(String.join(", ",
					importer.getSearchLocations().stream().map(f -> f.toString()).toArray(String[]::new)));
		}
	}
	...
	private void updateElements() {
		...
		try {
			...
			Activator.log(
					new Status(IStatus.ERROR, Activator.PLUGIN_ID, "An error occurred loading configurable items", e));
		}
	...
	protected Control createContents(Composite parent) {
		...
		extensionViewer.setComparator(new ViewerComparator() {
			@Override
		...
		statusColumn.setLabelProvider(new ColumnLabelProvider() {
			...
			public Image getImage(Object element) {
				if (((ConfigurableExtension) element).isConfigured()) {
					return Activator.getDefault().getImageRegistry().get(Activator.IMG_CHECKMARK);
				}
				return null;
			}
	...
	protected void doConfigure(final ConfigurableExtension extension) {
		try {
			...
			UiUtil.logAndDisplay(e.getStatus());
		}
	}
	...
	protected void doInstall(final ConfigurableExtension extension) {
		...
		if (!installDirectory.canWrite()) {
			UiUtil.logAndDisplay(new Status(IStatus.ERROR, Activator.PLUGIN_ID, NLS
					.bind("Installation failed. The directory ''{0}'' is not writeable.", userLocationText.getText())));
			return;
		}
		try {
			final IStatus[] status = new IStatus[1];
					...
					if (status[0].getSeverity() != IStatus.ERROR) {
						...
						if (status[0].getSeverity() != IStatus.OK) {
							MultiStatus result = new MultiStatus(Activator.PLUGIN_ID, 0,
									NLS.bind("The installation of {0} generated warning. See error log for details.",
											extension.getLabel()),
									null);
							result.add(status[0]);
